...
import { fetchTopAnime, searchAnime, fetchNewAnime } from "../../api/tmdbDashboard";
...
const DashboardAnime: React.FC<DashboardAnimeProps> = ({ selectedGenres, selectedRating }) => {
  ...
  useEffect(() => {
    withLoading(async () => {
      try {
        const results = await fetchTopAnime(language);
        setTopAnime(results);
        setVisibleCount(20);
      } catch (err) {
        console.error("Помилка завантаження топ-аніме:", err);
        toast.error(t("anime.loading_error") || "Помилка завантаження");
      }
    });
  }, [language]);
  ...
  useEffect(() => {
    ...
    setVisibleCount(20);
  }, [selectedGenres, selectedRating, topAnime, searchMode]);

  useEffect(() => {
    withLoading(async () => {
      try {
        const [nowPlaying, top] = await Promise.all([
          fetchNewAnime(language),
          fetchTopAnime(language),
        ]);
        setNewAnime(nowPlaying.slice(0, 10) as unknown as AnimeItem[]);
        setTopAnime(top);
        setVisibleCount(20);
      } catch (err) {
        console.error(err);
        toast.error(t("anime.loading_error"));
      }
    });
  }, [language]);
  ...
  const searchContent = async (term: string) => {
    ...
    await withLoading(async () => {
      try {
        setSearchMode(true);
        const results = await searchAnime(term.trim(), language, 1);
        setSearchResults(results.slice(0, 100));
        setVisibleCount(20);
        window.scrollTo({ top: 0, behavior: "smooth" });
      } catch (err) {
        console.error("Помилка пошуку аніме:", err);
        toast.error(t("anime.search_error") || "Помилка пошуку");
      }
    });
  };
  ...
  const loadMore = () => {
    const source = searchMode ? searchResults : topAnime;
    const nextCount = Math.min(visibleCount + 20, source.length);
    setVisibleCount(nextCount);
  };

  const renderGrid = () => {
    const items = searchMode ? searchResults : topAnime;
    ...
    return (
      <>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {visibleItems.map((item) => {
            ...
            const isWatchLater = watchLaterList.some(
              (m: any) => m.id === item.id && m.type === type
            );
        ...
        {visibleCount < (searchMode ? searchResults.length : topAnime.length) && (
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-10">
        ...
        {!searchMode && newAnime.length > 0 && (
          <SimpleHeroSlider movies={newAnime} />
        )}
        ...
        <SearchBar
          ...
          placeholder={t("anime.search_placeholder") || "Пошук аніме..."}
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black mb-10 text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">
          {searchMode
            ...
            : t("anime.top") || "Топ аніме"}
        </h1>

        {/* Контент */}
        {filteredAnime.length === 0 && !searchMode ? (
